%%
%% ACM Format: Mitigating GIL Bottlenecks in Edge AI Systems
%%
...
%% Suppress showhyphens warning from package conflict
\let\showhyphens\relax

%% ACM reference format (required for papers over one page)
\setcopyright{none}
\settopmatter{printacmref=true, printfolios=true}

%% Conference information
\acmConference[DOORS 2026]{International Conference on Designing and Operating Optimal Reliable Systems}{2026}{Zhytomyr, Ukraine}

\usepackage{algorithm}
\usepackage{algorithmic}

%% Allow flexible line breaking to avoid overfull hbox warnings
\sloppy
...
<concept_desc>Computer systems organization~Edge computing</concept_desc>
<concept_significance>500</concept_significance>
</concept>
<concept>
...
\ccsdesc[500]{Computer systems organization~Edge computing}
\ccsdesc[300]{Software and its engineering~Concurrency control}
\ccsdesc[300]{Computer systems organization~Embedded systems}

\keywords{Edge AI Systems, Runtime Optimization, GIL Contention, Adaptive Thread Pool, Blocking Ratio, Resource-Constrained Computing, Python Concurrency}

\maketitle

\section{Introduction}

Edge AI systems have converged on a standard architecture: Python-based orchestration (using frameworks like LangChain and LlamaIndex) managing high-performance inference kernels (PyTorch and TensorFlow Lite). The Python orchestration layer becomes a critical bottleneck. To mask I/O latency from network calls and sensors, developers reflexively increase thread counts. However, on resource-constrained edge devices, this triggers an adverse interaction with Python's Global Interpreter Lock (GIL), leading to what we term the ``saturation cliff''.
...
Our solution operates entirely in-process, making it suitable for memory constrained devices (512~MB--2~GB RAM) where multiprocessing incurs prohibitive overhead, while outperforming asyncio on mixed CPU and I/O workloads common in edge AI pipelines.
...
The combined instrumentation pattern (two calls to \texttt{time.time()} and two to \texttt{time.thread\_time()}) adds 0.30 $\mu$s median overhead per task. For our typical $T_{\text{CPU}} = 0.1$ ms workload, this represents less than 0.3\% overhead.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/fig_os_gil_paradox.png}
\Description{Three-panel diagram showing the OS-GIL Paradox: Panel A shows OS scheduler distributing threads across four CPU cores with blue boxes indicating scheduled threads. Panel B shows GIL serialization where one green thread holds the GIL while red threads are blocked. Panel C shows the contention cycle with arrows indicating rapid wakeup/sleep transitions causing context switch overhead.}
...
\begin{table}[htbp]
\centering
\caption{Saturation Cliff Across Configurations}
\label{tab:saturation_cliff}
\begin{tabular}{ccccc}
\toprule
\textbf{Threads} & \textbf{SC TPS} & \textbf{QC TPS} & \textbf{SC P99 (ms)} & \textbf{QC P99 (ms)} \\
\midrule
1    & 15,873 & 15,912 & 6.2 & 6.1 \\
32   & \textbf{37,437} & 67,523 & 10.1 & 9.1 \\
64   & 36,821 & \textbf{68,742} & 11.7 & 9.8 \\
256  & 31,087 & 59,421 & 38.2 & 23.7 \\
2048 & 25,386 & 45,821 & 412.8 & 289.4 \\
\midrule
\textbf{Loss} & \textbf{-32.2\%} & \textbf{-33.3\%} & \textbf{40.9$\times$} & \textbf{29.5$\times$} \\
\bottomrule
\end{tabular}
\end{table}

Peak throughput of 37,437 TPS is achieved at $N = 32$ threads for single-core, and 68,742 TPS at $N = 64$ threads for quad-core. At $N = 2048$, throughput degrades significantly in both cases.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/fig1_saturation_cliff.png}
\Description{Two-panel line graph showing the saturation cliff phenomenon. Panel (a) displays single-core throughput peaking at 32 threads then declining sharply. Panel (b) shows quad-core throughput peaking at 64 threads then declining. Both panels include an I/O baseline that scales linearly, contrasting with the mixed workload cliff.}
\caption{The Saturation Cliff. Panel (a) shows single-core throughput vs.\ threads; panel (b) shows quad-core. The I/O baseline scales linearly, confirming GIL contention causes the cliff.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/fig2_latency_analysis.png}
\Description{Line graph showing P99 tail latency versus thread count for single-core and quad-core configurations. Latency remains stable below 10ms at optimal thread counts, then increases exponentially beyond the saturation point, reaching over 400ms at 2048 threads for single-core and 289ms for quad-core.}
\caption{Latency Analysis. P99 latency explodes as thread count increases beyond the optimal point.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/fig_architecture.png}
\Description{Block diagram showing three interconnected components of the adaptive thread pool system: Instrumentor (capturing CPU and wall-clock time per task), Monitor (computing blocking ratio beta from aggregated readings), and Controller (modulating thread pool size). Dashed arrows show feedback paths between components, with the thread pool at the center receiving control signals.}
...
We must differentiate between waiting for I/O and contending for the GIL. We calculate a metric called the Blocking Ratio $\beta$. For a task $i$, let $t_{\text{CPU},i}$ be the CPU time and $t_{\text{wall},i}$ be the wall-clock time. Then:
...
\begin{enumerate}
\item \textbf{Instrumentor}: Records $t_{\text{CPU}}$ using \texttt{time.thread\_time()} and $t_{\text{wall}}$ using \texttt{time.time()}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/fig_controller_flow.png}
\Description{Flowchart showing the adaptive controller decision logic. Starting from queue length check, the flow branches based on blocking ratio beta comparison against threshold. If beta exceeds threshold, scale-up proceeds through hysteresis counter. If beta is below threshold, the GIL Safety Veto blocks scale-up. Scale-down occurs when queue is empty. The diagram highlights the veto mechanism as a key decision point.}
...
For edge AI systems running quantized models (50--500 MB), the memory overhead of ProcessPoolExecutor severely constrains available headroom. ThreadPoolExecutor enables concurrent I/O masking with negligible memory overhead, making our adaptive controller a practical solution for memory-constrained deployments.

\textbf{Asyncio Event Loop:} Pure async achieves excellent performance for I/O-bound workloads with minimal memory overhead ($<1$~MB). However, CPU phases block the event loop. At concurrency~256, asyncio achieved 15,467~TPS compared to our 18,056~TPS for the mixed workload.

\textbf{Queue Depth Scaler:} Traditional scalers that adjust thread count based on queue depth consistently overscale. Without $\beta$ awareness, a queue depth scaler with range $[4, 256]$ settled at 254 threads, achieving only 16,538 TPS versus our 18,056 TPS at 32 threads.

Table~\ref{tab:baseline_comparison} presents the comprehensive comparison across all baseline strategies.

\begin{table}[htbp]
\centering
\caption{Baseline Strategy Comparison (Mixed Workload). Memory column shows baseline harness footprint; incremental memory per spawned Python process is $\approx 20$ MB.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/fig6_baseline_comparison.png}
\Description{Bar chart comparing throughput (TPS) and memory usage across concurrency strategies: ThreadPool-32 and ThreadPool-256 achieve highest throughput around 17,000-18,000 TPS with minimal memory. ProcessPool-4 and ProcessPool-8 show lower throughput with significantly higher memory consumption (86-166 MB). Asyncio-128 and Asyncio-256 achieve moderate throughput with very low memory. QueueScaler shows intermediate performance.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{img/fig8_workload_heatmap.png}
\Description{Heatmap with workload types (I/O Heavy, I/O Dominant, Balanced, CPU Leaning, CPU Heavy) on the y-axis and thread counts (1 to 2048) on the x-axis. Color intensity represents throughput in TPS. I/O-heavy workloads show peak performance at higher thread counts (256), while CPU-heavy workloads show optimal performance at lower thread counts (32) with darker colors indicating throughput degradation at high thread counts.}
\caption{Workload Sweep Heatmap. Throughput (TPS) varies across workload types and thread counts. I/O-heavy workloads scale to higher thread counts; CPU-heavy workloads peak early.}
\label{fig:workload_heatmap}
\end{figure}

\subsection{Threshold Sensitivity Analysis}

To isolate threshold selection from mixed-workload variability, we conducted a parameter sweep using I/O-dominant workloads. Table~\ref{tab:threshold_sensitivity} shows that performance remains stable across $\beta_{\text{thresh}} \in [0.2, 0.7]$. The analysis confirms that $\beta_{\text{thresh}} = 0.3$ offers the best balance between responsiveness and stability, though the system is robust to parameter choice.
...
Table~\ref{tab:workload_generalization} demonstrates that the controller achieves 93.5\% average efficiency across diverse workload types without manual tuning. The blocking ratio $\beta$ correctly differentiates I/O-heavy tasks (Sensor Fusion, $\bar{\beta}=0.89$, $N=60$) from compute-heavy tasks (SLM Inference, $\bar{\beta}=0.21$, $N=24$). Notably, the Veto mechanism prevented an average of 9 scale-up attempts per workload that would have pushed the system into GIL contention.
...
John Ousterhout. 1996. Why Threads Are A Bad Idea (for most purposes). In \textit{Proc. USENIX Technical Conference}.

\bibitem{beazley2009}
David Beazley. 2009. Inside the Python GIL. Chicago Python User Group.

\bibitem{beazley2010}
David Beazley. 2010. Understanding the Python GIL. In \textit{Proc. PyCon}.

\bibitem{welsh2001}
Matt Welsh, David Culler, and Eric Brewer. 2001. SEDA: An Architecture for Well-Conditioned, Scalable Internet Services. In \textit{Proc. ACM SOSP}. 230--243.

\bibitem{delimitrou2014}
Christina Delimitrou and Christos Kozyrakis. 2013. Paragon: QoS-Aware Scheduling for Heterogeneous Datacenters. In \textit{Proc. ACM ASPLOS}. 77--88.

\bibitem{crankshaw2017}
Daniel Crankshaw, Xin Wang, Guilio Zhou, Michael J. Franklin, Joseph E. Gonzalez, and Ion Stoica. 2017. Clipper: A Low-Latency Online Prediction Serving System. In \textit{Proc. USENIX NSDI}. 613--627.

\bibitem{gross2023}
Sam Gross. 2023. PEP 703: Making the Global Interpreter Lock Optional in CPython. Python Enhancement Proposals. \url{https://peps.python.org/pep-0703/}

\bibitem{arm_cortex_a72}
ARM Ltd. 2015. Cortex-A72 Software Optimization Guide. ARM Documentation. \url{https://developer.arm.com/documentation/uan0016/a/}

\bibitem{gartner2018edge}
Santhosh Rao. 2018. What Edge Computing Means for Infrastructure and Operations Leaders. Gartner, Inc. \url{https://www.gartner.com/smarterwithgartner/what-edge-computing-means-for-infrastructure-and-operations-leaders}

\bibitem{iot_analytics2024}
IoT Analytics. 2024. Number of Connected IoT Devices Growing 14\% to 21.1 Billion Globally in 2025. IoT Analytics GmbH. \url{https://iot-analytics.com/number-connected-iot-devices/}

\bibitem{tiobe2024}
TIOBE Software. 2024. TIOBE Index for December 2024. \url{https://www.tiobe.com/tiobe-index/}

\bibitem{pypl2024}
Pierre Carbonnelle. 2024. PYPL PopularitY of Programming Language Index. \url{https://pypl.github.io/PYPL.html}

\bibitem{statista_ml2024}
Statista Research Department. 2024. Machine Learning Market Size Worldwide 2025-2030. Statista. \url{https://www.statista.com/statistics/1246443/machine-learning-market-size/}
